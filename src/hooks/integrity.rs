...
//! - Manual verification via `zap verify`
//!
...
pub enum IntegrityStatus {
    ...
    Verified,
    /// Hash mismatch — hook has been modified outside of `zap init`
...
/// Run integrity check and print results (for `zap verify` subcommand)
pub fn run_verify(verbose: u8) -> Result<()> {
    ...
    if !hook_path.exists() && !hash_file.exists() {
        ...
        println!("      Run `zap init -g` to install.");
        return Ok(());
    }
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            ...
            eprintln!();
            eprintln!("  The hook file has been modified outside of `zap init`.");
        ...
        IntegrityStatus::NoBaseline => {
            ...
            println!("      Run `zap init -g` to establish baseline.");
        }
        ...
        IntegrityStatus::OrphanedHash => {
            ...
            eprintln!("      Run `zap init -g` to reinstall.");
        }
    }
...
/// re-run `zap init -g --auto-patch` to re-establish the baseline.
