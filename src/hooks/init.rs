...
/// Template written by `zap init` when no filters.toml exists yet.
    ...
    if binary_hook_registered {
        ...
        #[cfg(unix)]
        {
            ...
            if !is_executable {
                ...
                println!(
                    "[warn] Hook: {} (outdated — run `zap init -g` to upgrade to native binary)",
                    hook_path.display()
                );
            } else if is_executable && has_guards {
                println!(
                    "[warn] Hook: {} (legacy script v{} — run `zap init -g` to upgrade)",
                    hook_path.display(),
                    hook_version
                );
            } else {
        ...
        {
            println!(
                "[warn] Hook: {} (legacy script — run `zap init -g` to upgrade)",
                hook_path.display()
            );
        }
    } else {
        ...
        if cursor_binary_registered {
            ...
            #[cfg(unix)]
            {
                ...
                if !is_executable {
                    ...
                    println!(
                        "[warn] Cursor hook: {} (legacy script — run `zap init -g --agent cursor` to upgrade)",
                        cursor_hook.display()
                    );
                }
            }
            ...
            #[cfg(not(unix))]
            {
                println!("[warn] Cursor hook: {} (legacy script — run `zap init -g --agent cursor` to upgrade)", cursor_hook.display());
            }
        } else {
...
/// Entry point for `zap init --gemini`
pub fn run_gemini(
    global: bool,
    hook_only: bool,
    patch_mode: PatchMode,
...
/// Entry point for `zap init --copilot`.
///
