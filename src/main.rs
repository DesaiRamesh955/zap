...
#[command(
    name = "zap",
    version,
    about = "Zap - Snip noisy command output before it hits your AI",
    long_about = "A high-performance CLI proxy that filters and summarizes system outputs before they reach your AI context. 60-90% fewer tokens, zero quality loss, runs entirely on-device."
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
enum Commands {
    ...
    /// Initialize zap instructions for assistant CLI usage
    Init {
        ...
        /// Remove Zap artifacts for the selected assistant mode
        #[arg(long)]
        uninstall: bool,

        /// Target Codex CLI (uses AGENTS.md + ZAP.md, no Claude hook patching)
        #[arg(long)]
        codex: bool,
    ...
    /// Claude Code economics: spending (ccusage) vs savings (zap) analysis
    CcEconomics {
    ...
    /// Discover missed Zap savings from Claude Code history
    Discover {
    ...
    /// Rewrite a raw command to its Zap equivalent (single source of truth for hooks)
    ///
    ...
    Rewrite {
        ...
        /// Accepts multiple args: `zap rewrite ls -al` is equivalent to `zap rewrite "ls -al"`
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
...
fn run_fallback(parse_error: clap::Error) -> Result<i32> {
    ...
    // e.g. `zap gain --badtypo` should show Clap's error, not try to run `gain` from $PATH.
    ...
    match command {
        PnpmCommands::Typecheck { .. } => {
            ...
            if !filters.is_empty() {
                ...
                let msg = format!(
                    "[zap] warning: --filter is not yet supported for pnpm {}, filters preceding the subcommand will be ignored",
                    cmd_name
                );
                return Some(msg);
            }
...
mod tests {
    ...
    fn test_rewrite_clap_multi_args() {
        // This is the bug KuSh reported: `zap rewrite ls -al` failed because
    ...
    fn test_rewrite_clap_quoted_single_arg() {
        // Quoted form: `zap rewrite "git status"` — single arg containing spaces
    ...
    fn test_pnpm_typecheck_with_filters() {
        ...
        match cli.command {
            Commands::Pnpm { filter, command } => {
                ...
                assert_eq!(filter, vec!["@app1", "@app2"]);
                assert_eq!(warning, "[zap] warning: --filter is not yet supported for pnpm tsc, filters preceding the subcommand will be ignored")
            }
            _ => panic!("Expected Pnpm Build command"),
        }
    }
