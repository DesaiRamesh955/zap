...
///
/// Serializable to JSON for export via `zap gain --daily --format json`.
///
/// # JSON Schema
///
/// ```json
...
///
/// Serializable to JSON for export via `zap gain --weekly --format json`.
...
///
/// Serializable to JSON for export via `zap gain --monthly --format json`.
#[derive(Debug, Serialize)]
...
impl Tracker {
    ...
    /// Get parse failure summary for `zap gain --failures`.
